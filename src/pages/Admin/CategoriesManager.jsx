...
export default function CategoriesManager() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingItem) {
        await api.put(`/admin/categories/${editingItem._id}`, payload);
                ...
                <tbody>
                  {filteredCategories.map((cat) => (
                    <tr key={cat._id}>
                      ...
                      <td style={{ color: 'var(--text-muted)', fontSize: '0.85rem' }}>{cat.slug}</td>
                      <td style={{ fontFamily: 'monospace', fontSize: '0.85rem' }}>{cat.icon || '-'}</td>
                      <td style={{ maxWidth: 250, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        {cat.description || '-'}
                      </td>
                      <td>
                        <span style={styles.badge(cat.isActive !== false ? 'success' : 'danger')}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end', marginTop: 8 }}>
                ...
                <button
                  ...
                  disabled={saving}
                >
                  <FaSave /> {saving ? 'Saving...' : editingItem ? 'Update Category' : 'Create Category'}
                </button>
              </div>
            </form>
          </div>
      ...
      {deleteTarget && (
        <div style={styles.deleteConfirmOverlay} onClick={() => setDeleteTarget(null)}>
          <div style={styles.deleteConfirmBox} onClick={(e) => e.stopPropagation()}>
            ...
            <h3 style={{ marginBottom: 8, fontFamily: 'var(--font-primary)' }}>Delete Category</h3>
