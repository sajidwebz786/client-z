                ...
                <tbody>
                  {filteredPayments.map((payment) => (
                    <tr key={payment._id}>
                      ...
                      <td>{payment.user?.email || payment.userEmail || '-'}</td>
                      <td>{payment.course?.title || payment.courseName || 'N/A'}</td>
                      <td style={{ color: 'var(--green-400)', fontWeight: 600 }}>
                        {formatCurrency(payment.amount)}
                      </td>
                      <td style={{ fontSize: '0.8rem', color: 'var(--text-muted)', fontFamily: 'monospace' }}>
                        {payment.razorpayOrderId || payment.razorpay_order_id || '-'}
                      </td>
                      <td>
                        <span style={styles.badge(getStatusBadge(payment.status))}>
